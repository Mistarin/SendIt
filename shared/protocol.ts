...
/** QR version 40 at the least expensive (L) ECC level. */
export const MAX_OPTICAL_FRAME_BYTES = 2953;
export const MAX_FILE_BYTES = 64 * 1024 * 1024;
// The optical frame carries the packed container, which can include a UTF-8
// filename and media type in addition to the file bytes. Keep this bound
// explicit so an untrusted QR code cannot make the receiver allocate based on
// an arbitrary u32 header value.
export const MAX_CONTAINER_BYTES = MAX_FILE_BYTES + 49 + 2 * 0xffff;
/**
...
export async function unpackFile(container: Uint8Array): Promise<OpticalFile> {
  ...
  if (container.length > MAX_CONTAINER_BYTES) throw new Error("The recovered file container is too large.");
...
export function packFrame(h: FrameHeader, block: Uint8Array): Uint8Array {
  if (
    !Number.isInteger(h.sessionId) || h.sessionId < 0 || h.sessionId > 0xffff ||
    !Number.isInteger(h.seq) || h.seq < 0 || h.seq > 0xffffffff ||
    !Number.isInteger(h.k) || h.k < 1 || h.k > 0xffff ||
    !Number.isInteger(h.blockLen) || h.blockLen < 1 ||
    !Number.isInteger(h.totalLen) || h.totalLen < 1 || h.totalLen > MAX_CONTAINER_BYTES ||
    block.length !== h.blockLen ||
    Math.ceil(h.totalLen / h.blockLen) !== h.k ||
    HEADER_LEN + h.blockLen > MAX_OPTICAL_FRAME_BYTES
  ) {
    throw new RangeError("Inconsistent or oversized optical frame.");
  }
...
export function parseFrame(
  ...
  if (header.totalLen > MAX_CONTAINER_BYTES) return null;
  // The sender always uses exactly ceil(totalLen / blockLen) source blocks.
  // Checking this before LTDecoder construction prevents malformed frames
  // from forcing large allocations or poisoning a live decoder.
  if (Math.ceil(header.totalLen / header.blockLen) !== header.k) return null;
  if (HEADER_LEN + header.blockLen > MAX_OPTICAL_FRAME_BYTES) return null;
