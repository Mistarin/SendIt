...
test("malformed optical containers are rejected", async () => {
  ...
  await assert.rejects(
    unpackFile(new Uint8Array(64 * 1024 * 1024 + 200_000)),
    /container is too large/,
  );
  const impossibleMetadata = new Uint8Array(49);
  impossibleMetadata.set([0x44, 0x43, 0x46, 0x32]);
  new DataView(impossibleMetadata.buffer).setUint16(5, 0xffff, true);
  await assert.rejects(unpackFile(impossibleMetadata), /length does not match/);
});
...
test("frames that are not ours, or not self-consistent, are rejected", () => {
  ...
  const impossibleBlocks = good.slice();
  new DataView(impossibleBlocks.buffer).setUint16(8, 2, true);
  assert.equal(parseFrame(impossibleBlocks), null, "block count must match total length");

  const oversized = good.slice();
  new DataView(oversized.buffer).setUint32(12, 0xffffffff, true);
  assert.equal(parseFrame(oversized), null, "declared payload must be bounded");

  const tooDense = good.slice();
  new DataView(tooDense.buffer).setUint16(10, MAX_OPTICAL_FRAME_BYTES, true);
  assert.equal(parseFrame(tooDense), null, "a frame larger than QR version 40 is not optical");
  assert.throws(
    () => packFrame({ sessionId: 1, seq: 0, k: 1, blockLen: 4, totalLen: 4, payloadFnv: 0 }, new Uint8Array(3)),
    /Inconsistent or oversized optical frame/,
  );
});
