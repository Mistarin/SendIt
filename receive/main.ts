...
const torchBtn = document.getElementById("cfg-torch") as HTMLButtonElement;
const zoomControl = document.getElementById("cfg-zoom") as HTMLInputElement;
...
/** Optional controls are capability-driven: unsupported browsers simply keep
 * the compact receiver UI and use their normal camera behavior. */
function setupCameraControls() {
  ...
  cameraControls.hidden = !hasTorch && !hasZoom;
  torchBtn.hidden = !hasTorch;
  zoomControl.parentElement!.hidden = !hasZoom;
  if (hasZoom && capabilities.zoom) {
    zoomControl.min = String(capabilities.zoom.min);
    zoomControl.max = String(capabilities.zoom.max);
    zoomControl.step = String(capabilities.zoom.step ?? 0.1);
    zoomControl.value = String(
      Math.max(capabilities.zoom.min, Math.min(capabilities.zoom.max, capabilities.zoom.min)),
    );
  }
}

async function applyCameraControl(constraints: MediaTrackConstraintSet) {
  const track = stream?.getVideoTracks()[0];
  if (!track) return;
  try {
    await track.applyConstraints({ advanced: [constraints] });
  } catch {
    cameraActual.textContent = "this camera refused that control — keeping the current setting";
  }
}
...
torchBtn.addEventListener("click", () => {
  ...
  void applyCameraControl({ torch: torchOn } as MediaTrackConstraintSet);
});
zoomControl.addEventListener("input", () => {
  void applyCameraControl({ zoom: Number(zoomControl.value) } as MediaTrackConstraintSet);
});

document.addEventListener("visibilitychange", () => {
  if (!document.hidden && !done) void requestScreenWakeLock();
});
 ...
 * staring at a blank receiver reaches for the phone. The defaults (1465 bytes
 * per frame at 24 fps) are tuned for ordinary phones, monitors, and longer
 * distances. Dense mode is the close-range alternative.
 *
...
function updateStats() {
  ...
  metric("m-busy").textContent = `${pool.busyCount}/${pool.size}`;
